...
async function challengeExit(originalIndex, plasmaBlockNumber, inputNumber, transaction, proof) {
    // function challengeExit(
    //     uint72 _utxoIndex, // one pending exit
    //     uint32 _plasmaBlockNumber, // block where the spend is
    //     uint8 _inputNumber,    // output being exited
    //     bytes _plasmaTransaction, // transaction itself
    //     bytes _merkleProof) // proof
    const transactionHex = ethUtil.bufferToHex(transaction);
    const proofHex = ethUtil.bufferToHex(proof);
    ...
    balanceBefore = new ethUtil.BN(balanceBefore); 
    let gas = await PlasmaContract.methods.challengeExit(originalIndex, plasmaBlockNumber, inputNumber, transactionHex, proofHex).estimateGas({from: account})
    console.log("Exit challenge gas price is " + gas);
    gas = 200000
    const result = await PlasmaContract.methods.challengeExit(originalIndex, plasmaBlockNumber, inputNumber, transactionHex, proofHex).send({from: account, gas: gas})
    let balanceAfter = await web3.eth.getBalance(account);
    balanceAfter = new ethUtil.BN(balanceAfter); 
...
async function main() {
    ...
    let am = "1000000000000000000"
    let inputs = [{blockNumber: 1, txNumberInBlock: 0, outputNumberInTransaction: 0, 
        amount: am // 1 ETH
    }];
    let outputs = [{to: bobAddress, amount: am}]
    let transaction = createTransaction(1, inputs, outputs, alice);
    let serializedTX = transaction.serialize()
    let sendingResult = await sendTransaction(ethUtil.bufferToHex(serializedTX), "127.0.0.1:3001")
    ...
    // spend from block number 2
    inputs = [{blockNumber: 2, txNumberInBlock: 0, outputNumberInTransaction: 0, 
        amount: am // 1 ETH
    }];
    outputs = [{to: aliceAddress, amount: am}];
    let doubleSpendTransaction = createTransaction(1, inputs, outputs, bob);
    let serializedDoubleSpendTX = doubleSpendTransaction.serialize()
    let doubleSendingResult = await sendTransaction(ethUtil.bufferToHex(serializedDoubleSpendTX), "127.0.0.1:3001")
    assert(doubleSendingResult.error === false);
    list = await getUTXOlist(bobAddress, "127.0.0.1:3001")
    assert(list.utxos.length === 0);

    // write block number 3
    hash = await getBlockHash(2, storage, BlockHeaderLength);
    hashFromContract = await getLastCommittedBlockHash();
    assert(hash.equals(hashFromContract));
    assembledBlock = await assembleBlock(3, hash, "127.0.0.1:3001");
    await storage.storeBlock(assembledBlock);
    parsedBlock = new Block(assembledBlock);
    // console.log(parsedBlock.from.toString('hex'));
    assert(parsedBlock.transactions.length === 1);
    ...
    if (result.action !== "sendChallenge") {
        console.log(result)
        throw Error("Fatal")
    }
    list = await getUTXOlist(bobAddress, "127.0.0.1:3001");
    ...
    let utxoIndex = exitStartResult.events.ExitStartedEvent.returnValues._index;
    let blockForChallenge = await storage.getBlock(Number.parseInt(result.blockForChallenge))
    let parsedBlockForChallenge = new Block(blockForChallenge)
    let info = parsedBlockForChallenge.getProofForTransactionByNumber(Number.parseInt(result.transactionForChallenge))
    assert(info !== null);

    let challengeResult = await challengeExit(utxoIndex, Number.parseInt(result.blockForChallenge), Number.parseInt(result.transactionForChallenge),
        info.tx.serialize(), info.proof);
